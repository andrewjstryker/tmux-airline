...
#
# Grammar: domain commands follow a noun-verb pattern. The read-only `version` and
# `help` commands are root leaves. Every noun's verbs live in a cmd_<noun> dispatcher
# below; help is generated from the `#| …` markers on the case arms, so the grammar
# documents itself.
...
cmd_classifier () {
  ...
  case "$verb" in
    describe)  catalog_describe classifier "$@" ;;      #| <classifier> — describe metadata, options, and resolved path
...
cmd_filter () {
  ...
  # help:begin filter
  case "$verb" in
    describe)  catalog_describe filter "$@" ;;      #| <filter> — describe metadata, options, and resolved path
...
cmd_probe () {
  ...
  # help:begin probe
  case "$verb" in
    describe)  runner_probe_describe "$@" ;;      #| <probe> — describe metadata, options, interval, and resolved path
...
#-----------------------------------------------------------------------------#
# Dispatch — public nouns delegate to their cmd_<noun> parser above. Read-only
# metadata/help are root leaves.
